...
- `references/pos-native-vs-extension.md` — Full capability map: what's native to POS Lite/Pro, what requires a third-party app, and what requires a custom UI Extension or is unsupported. Read this when a merchant's requirements (commissions, gift cards, item holds, age verification, etc.) may involve custom development — and flag scope risk accordingly.
- `references/pos-app-ecosystem.md` — POS-compatible apps by category (loyalty, returns, appointments, staff, reporting, delivery, gift cards, bundles, invoicing). Read this when reviewing the "Third-party apps installed" milestone or when a merchant's VTP lists integrations — verify each app is POS-compatible before assuming it works in-store.
